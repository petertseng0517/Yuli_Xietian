...
│   │   └── news.json       # 活動消息資料
│   └── img/
│       ├── hero1.jpg       # Hero 輪播圖 1（建議 1920×1080）
...
rsync -avz --exclude='.DS_Store' --exclude='.git' \
  temple-site/ root@伺服器IP:/var/www/temple-site/
```
...
- [ ] 廟宇正式全名
- [ ] Hero 輪播照片（`hero1.jpg`、`hero2.jpg`、`hero3.jpg`，建議 1920×1080）
- [ ] 廟宇介紹區照片（`about.jpg`）
- [ ] 主神照片（`deity.jpg`）
- [ ] 正確地址、聯絡電話
- [ ] Google Maps Embed URL（從 Google Maps 產生）
- [ ] 網域名稱確認
- [ ] 後台管理員密碼（部署後設定）
